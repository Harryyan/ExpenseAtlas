...
- **Conditional visibility: choose the right pattern based on the weight of the transition.**

  **Use `.opacity()` / `.overlay()` for minor UI toggles** (badges, labels, status indicators):
  - Preserves structural identity — SwiftUI diffs only the opacity property, not the subtree.
  - Keeps `@State` alive across toggles (e.g., scroll position, text field content).
  - Enables implicit animations between states.
  ```swift
  // Good — lightweight toggle
  Text("New").opacity(hasUnread ? 1 : 0)
  ```

  **Use `if/else` for major state transitions** (content switches, heavy subviews):
  - **Lazy loading** — heavy views (PDF rendering, CSV parsing, network-backed content) are not allocated until needed.
  - **Lifecycle cleanup** — `@State`, `onAppear`, and `task` reset correctly when the branch is destroyed, preventing stale data.
  - **Accessibility** — `if/else` removes the hidden branch from the VoiceOver tree automatically. `.opacity(0)` leaves invisible elements navigable, requiring manual `.accessibilityHidden(true)`.
  ```swift
  // Good — major content switch with heavy subviews
  if let doc {
      DocumentPreviewView(doc: doc)   // PDF/CSV rendering, lifecycle-dependent
  } else {
      ContentUnavailableView("Select a document", ...)
  }
  ```

  **Decision checklist — use `if/else` when ANY of these apply:**
  1. The hidden branch contains expensive views (media players, document renderers, large lists).
  2. You need `@State` or `onAppear` to reset when switching between branches.
  3. The hidden content should not be reachable by VoiceOver.

  Otherwise, default to `.opacity()` / `.overlay()` for a stable view tree.
